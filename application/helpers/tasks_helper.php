...
function getQtyItems($project_id, $task_id)
{
    $array_commodity=[];
    $index=0;
    $CI = &get_instance();

    $sql ='SELECT SUM(quantities) as quantities, commodity_name, commodity_code FROM '.db_prefix().'goods_delivery_detail where subproject = "'.$project_id.'" AND task = "'.$task_id.'" group by commodity_code';
    $array_data = $CI->db->query($sql)->result_array();

    if(count($array_data)>0){
        foreach ($array_data as $commodity_id) {
            $array_commodity[$index]['commodity_name'] = $commodity_id['commodity_name'];
            $array_commodity[$index]['quantities'] = $commodity_id['quantities'];
            $index++;
        }
    }
...
function is_total_expenses($project_id, $task_id)
{
    $CI = &get_instance();
	$CI->db->where('project_id', $project_id);
    $CI->db->where('expense_task_id', $task_id);
    $CI->db->select('SUM(amount) as total_money')->from(db_prefix() . 'expenses');
    $query           = $CI->db->get();
    $money = $query->row()->total_money;
    if ($money == '') {
        return '';
    }
...
function is_total_money($project_id, $task_id)
{
    $CI = &get_instance();
	$CI->db->where('project', $project_id);
    $CI->db->select('SUM(total_money) as total_money')->from(db_prefix() . 'goods_delivery');
    $query           = $CI->db->get();
    $money = $query->row()->total_money;
    if ($money == '') {
        return '';
