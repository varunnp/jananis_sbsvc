...
*/
            ...
            if (is_admin()) {
                $row[] = $aRow['client_cost'];
                $row[] = $aRow['unit_rate'];
                $row[] = $aRow['total_qty'];
                $row[] = $aRow['unit_qty'];
                $row[] = $aRow['qty_measures'];
                $row[] = $aRow['allocated_budget'];
                
                $allocatedbudget =   $aRow['allocated_budget'];  
                
                $amt = is_total_money($rel_id, $aRow['id']);
                if($amt == ''){
                    $amt = 0;
                }
                $amt1 = is_total_expenses($rel_id, $aRow['id']);
                if($amt1 == ''){
                    $amt1 = 0;
                }
                ...
                $balance = $allocatedbudget - $amt - $amt1;
                $row[] = $balance;
                
                if($allocatedbudget != 0 && $balance != 0){
                    $res = ($balance/$allocatedbudget)*100;
                }else{
                    $res = 0;
                }
