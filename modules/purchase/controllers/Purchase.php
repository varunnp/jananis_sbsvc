...
class purchase extends AdminController
{
    ...
    public function purchase_request(){
        ...
        if($this->input->get('group')){
            ...
            $this->load->view('purchase_request/price_list_manage', $data);
        }else{
    ...
    public function price_list_request($id = ''){
    	$this->load->model('departments_model');
        $this->load->model('staff_model');
        $this->load->model('projects_model');
        $this->load->model('currencies_model');
    	if($id == ''){
    		...
    		if($this->input->post()){
    			...
    			redirect(admin_url('purchase/purchase_request?group=price_list'));
    		}
        ...
        $this->load->view('purchase_request/price_list_request', $data);
    }
    ...
    public function quotations($id = ''){
        ...
        if($this->input->get('group')){
            $this->load->view('quotations/vendor_price_manage', $data);
        }else{
    ...
    public function vendor_price($id = '')
    {
        $this->load->model('currencies_model');
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                if ($id) {
                    ...
                    redirect(admin_url('purchase/quotations?group=vendor_price' . $id));
        ...
        $data['title']             = $title;
        $this->load->view('quotations/vendor_price', $data);
    }
     ...
     *  @return json
     */
    public function import_goods_report1()
    {
        if ($this->input->is_ajax_request()) {
            $this->load->model('currencies_model');
    ...
    public function import_goods_report()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $select = [
                'tblitems.commodity_code as item_code', 
                'tblitems.description as item_name',
                'tblpur_estimates.date as date',
                'total_money',
            ];
            $where =[];
            $custom_date_select = $this->get_where_report_period(db_prefix() . 'pur_estimates.date');
            if ($custom_date_select != '') {
                array_push($where, $custom_date_select);
            }
            $currency = $this->currencies_model->get_base_currency();
            ...
            if($this->input->post('report_currency')){
                ...
                if($report_currency == $base_currency->id){
                    array_push($where, 'AND '.db_prefix().'pur_estimates.currency IN (0, '.$report_currency.')');
                }else{
                    array_push($where, 'AND '.db_prefix().'pur_estimates.currency = '.$report_currency);
                }
            ...
            $aColumns     = $select;
            $sIndexColumn = 'id';
            $sTable       = db_prefix() . 'pur_estimate_detail';
            $join         = [
                'LEFT JOIN ' . db_prefix() . 'items ON ' . db_prefix() . 'items.id = ' . db_prefix() . 'pur_estimate_detail.item_code',
                'LEFT JOIN ' . db_prefix() . 'pur_estimates ON ' . db_prefix() . 'pur_estimates.id = ' . db_prefix() . 'pur_estimate_detail.pur_estimate',
            ];

            $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
                db_prefix() . 'items.id as item_id',
                db_prefix() . 'pur_estimate_detail.pur_estimate as po_id'
            ]);
