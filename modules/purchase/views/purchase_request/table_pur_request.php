...
if ($this->ci->input->post('req_type')
    && $this->ci->input->post('req_type') != '') {
    array_push($where, 'AND req_type = "price_list"');
}else{
    array_push($where, 'AND req_type = ""');
}
   ...
   for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if($aColumns[$i] == 'request_date'){
            ...
            if ( (has_permission('purchase_request', '', 'edit') || is_admin()) &&  $aRow['status'] != 2) {
                ...
                if($this->ci->input->post('req_type') != ''){
                    $name .= ' | <a href="' . admin_url('purchase/price_list_request/' . $aRow['id'] ).'" >' . _l('edit') . '</a>';
                }else{
