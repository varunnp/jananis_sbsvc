...
function get_staff_roject($staff_id,$project_id, $from_date, $to_date){
    $project = '';
    $CI = &get_instance();
    $custom_date_select = '';
    if($from_date != '' &&  $to_date != ''){
       /* $custom_date_select .= ' AND (tbltaskstimers.start_time between "' .
				date('Y-m-d', strtotime(date($from_date))) .
			//	'" and "' .
				//date('Y-m-d', strtotime(date($to_date))) . '")';*/
		$custom_date_select .= "  AND FROM_UNIXTIME(start_time) BETWEEN '$from_date' AND '$to_date' ";		
    ...
    if($project_id != ''){
        ...
        $row = $CI->db->query('SELECT tblprojects.name as project_name, tbltaskstimers.start_time FROM  tblprojects left join tbltasks ON tbltasks.rel_id = tblprojects.id LEFT JOIN   tbltaskstimers ON tbltaskstimers.task_id = tbltasks.id WHERE tbltasks.rel_type = "project" AND tblprojects.id = '.$project_id.'  AND tbltaskstimers.staff_id =  '.$staff_id.$custom_date_select)->result_array();
    	if($row){
    	    $project .=  '<span>'.$row[0]['project_name'].'</span><br>( ';
    	    foreach($row as $res){
    	        $project .=  date('d-m-Y', $res['start_time']).', ';
    	    }
    	    $project .= ' )';
    	    return $project;
    	}else{
        ...
        $row = $CI->db->query('SELECT tblprojects.name as project_name, tbltaskstimers.start_time FROM  tblprojects left join tbltasks ON tbltasks.rel_id = tblprojects.id LEFT JOIN   tbltaskstimers ON tbltaskstimers.task_id = tbltasks.id WHERE tbltasks.rel_type = "project" AND tbltaskstimers.staff_id =  ' . $staff_id.$custom_date_select)->result_array();
    	if($row){
    	    foreach($row as $res){
    	        $project .=  '<span>'.$res['project_name'].'</span>('.date('d-m-Y', $res['start_time']).')<br>';
    	    }
    	    return $project;
    	}else{
    	    return '';
