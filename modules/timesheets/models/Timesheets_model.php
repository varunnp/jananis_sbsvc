...
class timesheets_model extends app_model {
	...
	public function getAdvanceAmt($staffid, $from_date, $to_date){
	    
	    $this->db->select('SUM(amount) as amount');
	    $this->db->where("(date BETWEEN '$from_date' and '$to_date')");
        $this->db->where('clientid = ' . $staffid );
        return $this->db->get(db_prefix() . 'receiptrecords')->row();
	}
 ...
 */
	 ...
	 */
	public function get_taskstimers($staff_id, $where,$project_id = '') {
		$this->db->where($where);
		$this->db->where('staff_id', $staff_id);
		if($project_id != ''){
 ...
 * @return array
 */
	public function get_attendance_task($staffs_list, $month = '', $year = '', $from_date = '', $to_date = '',$project_id = '') {
		$data['staff_row_tk'] = [];
		...
		foreach ($staffs_list as $s) {
			...
			$list_taskstimers = $this->get_taskstimers($s['staffid'], $taskstimesWhere,$project_id);
			$dt_ts = [];
			$dt_ts_detail = [];
