 ...
 */
	public function reports() {
		...
		$data['department'] = $this->departments_model->get();
		$data['roles'] = $this->roles_model->get();
		$data['projects'] = $this->timesheets_model->get_projects();
		$data['workplace'] = $this->timesheets_model->get_workplace();
		...
		if ($this->input->is_ajax_request()) {
			if ($this->input->post()) {
				...
				if ($data_timekeeping_form == 'timekeeping_task') {
					...
					$data_timesheet = $this->timesheets_model->get_attendance_task($rResult, '', '', $from_date, $to_date,$project_filter);
				} else {
					$data_timesheet = $this->timesheets_model->get_attendance_manual($rResult, '', '', $from_date, $to_date,$project_filter);
				}
				...
				if ($data_timesheet) {
					...
					foreach ($rResult as $aRow) {
						...
						$row[] = $aRow['firstname'];
						$row[] = $aRow['lastname'];
						$row[] = get_staff_role($aRow['staffid']);
						$row[] = get_staff_roject($aRow['staffid'],$project_filter,$from_date, $to_date);
						$row[] = $aRow['hourly_rate'];
						$totaldaily_rate += $aRow['hourly_rate'];
						$total = 0;
						$total2 = 0;
						$total3 = 0;
						$total7 = 0;
						...
						$totalshift_days += $total_shift;
						
						$working_days = ($total > 0) ? (float) number_format($total, 2) : 0;
						$row[] = $working_days;
						$total_working_days += $working_days;
						$salary = $aRow['hourly_rate'] * $working_days;
						$row[] = $aRow['hourly_rate'] * $working_days;
						
						$total_salarys += $salary;
						
						
						$advance = $this->timesheets_model->getAdvanceAmt($aRow['staffid'], $from_date, $to_date);
						$advance_amt =  ($advance->amount != '') ? $advance->amount : 0;
						$row[] = $advance_amt;
						
						$total_advance += $advance_amt;
						
						$row[] = $salary - $advance_amt;
						$netsalary = $salary - $advance_amt;
					...
					$rows[] = '';
					$rows[] = 'total';
					$rows[] = $totaldaily_rate;
					$rows[] = $totalshift_days;
					$rows[] = $total_working_days;
					$rows[] = $total_salarys;
					$rows[] = $total_advance;
					$rows[] = $total_net_salary;
					...
					$rows1[] = '';
					$rows1[] = '';
					$rows1[] = 'Total days between fromdate & todate';
					$rows1[] = $days * -1;
					$rows1[] = $from_date;
					$rows1[] = $to_date;
					$rows1[] = '';
